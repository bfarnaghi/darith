...
def main():
    ...
    with sync_playwright() as playwright:
        ...
        page.get_by_role("link", name="Tutorial").first.wait_for()
        assert_no_horizontal_overflow(page)
        page.screenshot(path=OUTPUT_DIR / "landing-desktop.png", full_page=True)

        page.get_by_role("link", name="Tutorial").first.click()
        page.wait_for_url("**/tutorial/")
        page.get_by_role("heading", name="Understand your money, one section at a time.").wait_for()
        page.get_by_role("heading", name="Overview", exact=True).wait_for()
        page.get_by_text("How future income is treated").wait_for()
        assert_no_horizontal_overflow(page)
        page.screenshot(path=OUTPUT_DIR / "tutorial-desktop.png", full_page=True)
